...
import { fetchDataPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [fetchDataPlugin],
    });

    await request(server).get('/api/fetch-data/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/fetch-data/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/fetch-data/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/fetch-data/todos/${createdTodoItem.id}`)
